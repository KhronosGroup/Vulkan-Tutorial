...
void SetupScene(Engine *engine)
{
	...
	if (auto *renderer = engine->GetRenderer())
	{
		...
		// Defer switching to Textures phase until the first texture job is actually scheduled,
		// so the overlay doesn’t sit at 0% with (0/0) for an extended period.
	}
	std::thread([engine] {
...
int main(int argc, char *argv[])
{
	try
	{
		...
		bool debugSync = std::find(args.begin(), args.end(), "--debug-sync") != args.end();
		...
		// Initialize the engine
		if (!engine.Initialize("Sync2 Engine", WINDOW_WIDTH, WINDOW_HEIGHT, ENABLE_VALIDATION_LAYERS, debugSync))
		{
			throw std::runtime_error("Failed to initialize engine");
		}
