...
class Renderer {
  public:
	 ...
	 */
    bool Initialize(const std::string& appName, bool enableValidationLayers = true, bool useXR = false);
    ...
    void Render(const std::vector<Entity *>& entities, CameraComponent* camera, ImGuiSystem* imguiSystem, XrTime predictedTime);
    ...
    void SetAndroidApp(struct android_app* app) { xrContext.setAndroidApp(app); }
#endif
  ...
  private:
    ...
    void updateUniformBuffer(uint32_t currentFrame, uint32_t eye, const glm::mat4& view, const glm::mat4& proj, const glm::vec3& camPos);
    ...
    void drawRenderJob(const vk::raii::CommandBuffer& cmd, const RenderJob& job, uint32_t currentFrame, uint32_t eye, bool transparent);
