...
class CameraComponent : public Component
{
  ...
  private:
	...
	// Stereo Matrices (Chapter 5)
	std::array<glm::mat4, 2> eyeViewMatrices       = {glm::mat4(1.0f), glm::mat4(1.0f)};
	std::array<glm::mat4, 2> eyeProjectionMatrices = {glm::mat4(1.0f), glm::mat4(1.0f)};
  ...
  public:
	 ...
	 */
	void SetClipPlanes(float nearDist, float farDist)
	{
		nearPlane             = nearDist;
		farPlane              = farDist;
		projectionMatrixDirty = true;
	...
	/**
	 * @brief Set the stereo views for XR (Chapter 5).
	 * @param left The left eye view data.
	 * @param right The right eye view data.
	 */
	void SetStereoViews(const XrView& left, const XrView& right);

	/**
	 * @brief Get the eye-specific view matrix.
	 * @param eye The eye index (0 for left, 1 for right).
	 * @return The view matrix.
	 */
	const glm::mat4& GetViewMatrix(uint32_t eye) const { return eyeViewMatrices[eye]; }

	/**
	 * @brief Get the eye-specific projection matrix.
	 * @param eye The eye index (0 for left, 1 for right).
	 * @return The projection matrix.
	 */
	const glm::mat4& GetProjectionMatrix(uint32_t eye) const { return eyeProjectionMatrices[eye]; }
