...
bool MemoryPool::initialize() {
  ...
  try {
    ...
    // Use 1KB alignment (multiple of 64B nonCoherentAtomSize) to reduce search space
    configurePool(
      PoolType::UNIFORM_BUFFER,
      4 * 1024 * 1024,
      // 4MB blocks
      1024,
      // 1KB allocation units
...
std::unique_ptr<MemoryPool::MemoryBlock> MemoryPool::createMemoryBlock(PoolType poolType, vk::DeviceSize size, vk::MemoryAllocateFlags allocFlags) {
  ...
  if (allocFlags & vk::MemoryAllocateFlagBits::eDeviceAddress) {
...
std::pair<MemoryPool::MemoryBlock *, size_t> MemoryPool::findSuitableBlock(PoolType poolType, vk::DeviceSize size, vk::DeviceSize alignment) {
  ...
  for (const auto& block : poolBlocks) {
    ...
    size_t i = 0;
    while (i < totalUnits) {
      // Fast skip over occupied units
      if (!block->freeList[i]) {
        // Find the next free unit efficiently
        auto it = std::find(block->freeList.begin() + i, block->freeList.end(), true);
        if (it == block->freeList.end()) {
          break; // No more free units in this block
        }
        i = std::distance(block->freeList.begin(), it);
      }
      ...
      // If we found a 'false' at 'j', skip past it!
      // If we didn't find enough 'true's but j is still totalUnits, we're done with this block.
      i = (j > i) ? j : (i + 1);
    }
  }
  ...
  try {
    auto newBlock = createMemoryBlock(poolType, alignedSize, config.allocFlags);
    auto& poolBlocks = pools[poolType];
...
std::pair<vk::raii::Buffer, std::unique_ptr<MemoryPool::Allocation>> MemoryPool::createBuffer(
  ...
  // Normal pooled allocation path for all buffers, including those with device address support.
  // The pools are pre-configured with VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT where needed.
