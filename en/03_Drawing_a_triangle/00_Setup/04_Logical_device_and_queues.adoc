...
[source,multilang,c++,c]
.Finding the graphics queue family
----
// START c++
...
for (; graphicsIndex < queueFamilyCount; ++graphicsIndex)
{
    if (queueFamilyProperties[graphicsIndex].queueFlags & VK_QUEUE_GRAPHICS_BIT)
    {
        break;
    }
}
...
// START c
// In C, the pNext chain has to be linked together by hand, from the last
// structure back to the first.
...
featureChain.pNext = &vulkan11Features;
// END c
----
...
[source,multilang,c++,c]
.Retrieving the graphics queue handle
----
// START c++
