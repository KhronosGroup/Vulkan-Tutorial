...
std::string Renderer::ResolvePath(const std::string& filename) const {
  ...
  // If it's already an absolute path, just check if it exists and return it
  ...
  std::vector<std::string> searchPaths = {
    ...
    "../../simple_engine/cmake-build-debug/" + filename,
    "sync2_engine/cmake-build-debug/" + filename,
    "sync2_engine/build/" + filename,
    "../../simple_engine/build/" + filename,
    "simple_engine/cmake-build-debug/" + filename,
    "../sync2_engine/cmake-build-debug/" + filename,
    "../sync2_engine/build/" + filename,
    "Assets/" + filename,
  ...
  for (const auto& path : searchPaths) {
    try {
      ...
      // Ignore errors from weird path combinations
    }
  }
...
std::vector<char> Renderer::readFile(const std::string& filename) {
  ...
  std::cerr << "CRITICAL: Failed to open file: " << resolvedPath << " (original name: " << filename << ")" << std::endl;
...
vk::Extent2D Renderer::chooseSwapExtent(const vk::SurfaceCapabilitiesKHR& capabilities) {
  ...
  if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
    ...
    if (!platform) {
        std::cerr << "CRITICAL: platform pointer is NULL in chooseSwapExtent!" << std::endl;
        throw std::runtime_error("platform pointer is NULL");
    }
    int width, height;
    ...
    // Manual clamp to avoid std::clamp issues if any
...
void Renderer::WaitIdle() {
  ...
  // 2. Also wait for uploads and frame timeline semaphores if they exist
  auto waitTimeline = [&](vk::raii::Semaphore& sem, std::atomic<uint64_t>& nextValAtomic) {
    if (*sem) {
      uint64_t target = nextValAtomic.load(std::memory_order_relaxed);
      if (target > 0) {
        while (true) {
  ...
  waitTimeline(uploadsTimeline, nextUploadTimelineValue);
  waitTimeline(frameTimeline, nextFrameTimelineValue);
...
vk::Result Renderer::waitForFencesSafe(const std::vector<vk::Fence>& fences, vk::Bool32 waitAll, uint64_t timeoutNs) {
  ...
  while (true) {
    ...
    if (r == vk::Result::eSuccess)
      return vk::Result::eSuccess;
    if (r == vk::Result::eTimeout) {
      // We do NOT kick the watchdog here. If the fence wait takes 10+ seconds,
      // it means the GPU has legitimately hung or stalled, and we WANT the
      // watchdog to abort and report it.
      continue;
    }
    return r;
  }
