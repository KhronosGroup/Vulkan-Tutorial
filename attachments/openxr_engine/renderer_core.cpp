...
bool Renderer::Initialize(const std::string& appName, bool enableValidationLayers, bool useXR) {
  xrMode = useXR;

  // Initialize OpenXR early to get required Vulkan extensions
  if (xrMode) {
    if (!xrContext.createInstance(appName)) {
...
bool Renderer::createLogicalDevice(bool enableValidationLayers) {
  try {
    ...
    if (xrMode) {
      if (!xrContext.createSession(*physicalDevice, *device, queueFamilyIndices.graphicsFamily.value(), 0)) {
