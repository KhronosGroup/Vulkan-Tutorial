...
void PhysicsSystem::Update(std::chrono::milliseconds deltaTime, uint64_t timelineValue, uint32_t frameIndex) {
  ...
  std::vector<PendingCreation> toCreate; {
    ...
    if (!pendingCreations.empty()) {
      // Time-slice creations to avoid massive frame hangs on huge scene loads
      // Reduced to 20 to keep the progress bar moving smoothly and UI responsive.
      const size_t maxCreationsPerFrame = 20;
      size_t count = std::min(pendingCreations.size(), maxCreationsPerFrame);
  ...
  for (const auto& pc : toCreate) {
    if (++processed % 5 == 0 && renderer) {
      renderer->KickWatchdog();
    }
    if (!pc.entity)
      continue;
  ...
  if (initialized&& gpuAccelerationEnabled && renderer && canUseGPUPhysics) {
    SimulatePhysicsOnGPU(deltaTime, timelineValue, frameIndex);
  } else {
    // NO CPU FALLBACK - GPU physics must work, or physics is disabled.
    // IMPORTANT: Even if physics is disabled, we MUST signal the timeline milestone
    // so the renderer (which waits for physics) does not deadlock.
    if (renderer && timelineValue > 0) {
      // Only signal if the value strictly advances the frame timeline; otherwise skip to avoid
      // out-of-order (smaller-than-current) signals that violate VVL timeline rules.
      if (timelineValue > renderer->GetCurrentTimelineValue()) {
        renderer->SignalFrameTimeline(timelineValue);
      }
    }
...
static std::vector<char> readFile(const std::string& filename) {
  std::vector<std::string> searchPaths = {
    ...
    "cmake-build-release/" + filename,
    "build/" + filename,
    "../simple_engine/" + filename,
    "../simple_engine/cmake-build-debug/" + filename,
    "../" + filename,
    "../../" + filename
  };
  ...
  std::cerr << "CRITICAL: physics_system::Failed to open " << filename << " in any of the following paths:" << std::endl;
  for (const auto& path : searchPaths) {
    std::cerr << "  - " << path << std::endl;
  }

  throw std::runtime_error("Failed to open file in any search path: " + filename);
}
...
bool PhysicsSystem::InitializeVulkanResources() {
  ...
  try {
    ...
    commandBufferInfo.commandBufferCount = static_cast<uint32_t>(renderer->GetMaxFramesInFlight());

    try {
      vulkanResources.commandBuffers = raiiDevice.allocateCommandBuffers(commandBufferInfo);
    } catch (const std::exception& e) {
...
void PhysicsSystem::CleanupVulkanResources() {
  ...
  // 6. Destroy the command buffers before the command pool
  vulkanResources.commandBuffers.clear();
...
void PhysicsSystem::UpdateGPUPhysicsData(std::chrono::milliseconds deltaTime) const {
  ...
  if (!rigidBodies.empty()) {
    ...
    for (size_t i = 0; i < count; i++) {
      auto* concreteRigidBody = dynamic_cast<ConcreteRigidBody *>(rigidBodies[i].get());
      if (!concreteRigidBody) {
        continue;
      }

      const bool isKinematic = concreteRigidBody->IsKinematic();
      if (!isKinematic) {
        dynamicCount++;
      }

      // Skip GPU updates for static (kinematic) bodies that haven't changed.
      // Dynamic bodies (like tennis balls) always need their state synchronized.
      if (isKinematic && !concreteRigidBody->gpuNeedsUpdate) {
        continue;
      }
      concreteRigidBody->gpuNeedsUpdate = false;
      ...
      switch (concreteRigidBody->GetShape()) {
        ...
        case CollisionShape::Mesh: {
          ...
          auto* entity = concreteRigidBody->GetEntity();
          if (entity) {
  ...
  hasDynamicBodies = (dynamicCount > 0);
...
void PhysicsSystem::SimulatePhysicsOnGPU(const std::chrono::milliseconds deltaTime, uint64_t timelineValue, uint32_t frameIndex) const {
  if (!renderer) {
    fprintf(stderr, "SimulatePhysicsOnGPU: No renderer available");
    return;
  }

  const uint32_t numFrames = static_cast<uint32_t>(vulkanResources.commandBuffers.size());
  if (numFrames == 0) return;
  uint32_t actualFrameIndex = frameIndex % numFrames;
  ...
  vulkanResources.commandBuffers[actualFrameIndex].reset();

  // Begin command buffer
  vk::CommandBufferBeginInfo beginInfo;
  beginInfo.flags = vk::CommandBufferUsageFlagBits::eOneTimeSubmit;

  vulkanResources.commandBuffers[actualFrameIndex].begin(beginInfo);

  vulkanResources.commandBuffers[actualFrameIndex].bindDescriptorSets(
  ...
  // MODERN BARRIER: Use vk::DependencyInfo and vk::BufferMemoryBarrier2 (Synchronization 2)
  // Ensures host-written data is visible to compute shaders.
  vk::BufferMemoryBarrier2 hostToDeviceBarrier{
    ...
    .buffer = *vulkanResources.physicsBuffer,
    .offset = 0,
    .size = VK_WHOLE_SIZE
  };
  vk::DependencyInfo depInfo{.bufferMemoryBarrierCount = 1, .pBufferMemoryBarriers = &hostToDeviceBarrier};
  vulkanResources.commandBuffers[actualFrameIndex].pipelineBarrier2(depInfo);

  // Step 1: Integrate forces and velocities
  vulkanResources.commandBuffers[actualFrameIndex].bindPipeline(vk::PipelineBindPoint::eCompute, *vulkanResources.integratePipeline);
  vulkanResources.commandBuffers[actualFrameIndex].dispatch(static_cast<uint32_t>((rigidBodies.size() + 63) / 64), 1, 1);

  // MODERN BARRIER: Ensure integration is complete before collision detection
  ...
  depInfo.pBufferMemoryBarriers = &computeBarrier;
  vulkanResources.commandBuffers[actualFrameIndex].pipelineBarrier2(depInfo);

  // Step 2: Broad-phase collision detection
  // Optimization: skip broad-phase if no dynamic bodies exist to avoid O(N^2) GPU cost
  if (hasDynamicBodies || needsBroadPhase) {
    vulkanResources.commandBuffers[actualFrameIndex].bindPipeline(vk::PipelineBindPoint::eCompute, *vulkanResources.broadPhasePipeline);
    uint32_t nBodies = static_cast<uint32_t>(std::min(rigidBodies.size(), static_cast<size_t>(maxGPUObjects)));
    uint32_t numPairs = (nBodies * (nBodies - 1)) / 2;
    uint32_t broadPhaseThreads = (numPairs + 63) / 64;
    vulkanResources.commandBuffers[actualFrameIndex].dispatch(std::max(1u, broadPhaseThreads), 1, 1);
    needsBroadPhase = false;
  }

  // MODERN BARRIER: Broad phase to Narrow phase
  vulkanResources.commandBuffers[actualFrameIndex].pipelineBarrier2(depInfo);

  // Step 3: Narrow-phase collision detection
  vulkanResources.commandBuffers[actualFrameIndex].bindPipeline(vk::PipelineBindPoint::eCompute, *vulkanResources.narrowPhasePipeline);
  uint32_t narrowPhaseThreads = (maxGPUCollisions + 63) / 64;
  vulkanResources.commandBuffers[actualFrameIndex].dispatch(narrowPhaseThreads, 1, 1);

  // MODERN BARRIER: Narrow phase to Resolution
  vulkanResources.commandBuffers[actualFrameIndex].pipelineBarrier2(depInfo);

  // Step 4: Collision resolution
  vulkanResources.commandBuffers[actualFrameIndex].bindPipeline(vk::PipelineBindPoint::eCompute, *vulkanResources.resolvePipeline);
  uint32_t resolveThreads = (maxGPUCollisions + 63) / 64;
  vulkanResources.commandBuffers[actualFrameIndex].dispatch(resolveThreads, 1, 1);

  // End command buffer
  vulkanResources.commandBuffers[actualFrameIndex].end();

  // Submit the command buffer with timeline semaphore for non-blocking coordination
  vk::CommandBuffer cmdBuffer = *vulkanResources.commandBuffers[actualFrameIndex];
  if (timelineValue > 0) {
    renderer->SubmitToComputeQueue2(cmdBuffer, renderer->GetFrameTimeline(), timelineValue);
  } else {
    // Fallback to legacy submit if no timeline provided (should not happen in Sync2Engine)
    const vk::raii::Device& device = renderer->GetRaiiDevice();
    device.resetFences(*vulkanResources.computeFence);
    renderer->SubmitToComputeQueue(cmdBuffer, *vulkanResources.computeFence);
    ReadbackGPUPhysicsData(); // Legacy path blocks on CPU
  }
}
