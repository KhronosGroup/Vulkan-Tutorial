...
usually best to do the math by hand rather than depend upon the offsetof
C macro for C{pp} types.  Offsetof tends to quickly get one into
undefined behavior when dealing with polymorphism (it's a C macro not a C{pp}
function).
