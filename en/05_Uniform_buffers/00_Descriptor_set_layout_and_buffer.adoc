...
[source,multilang,c++,c]
.Descriptor set layout class member
----
// START c++
...
[source,multilang,c++,c]
.Referencing the descriptor set layout in the pipeline layout
----
// START c++
...
Go back to the `vk::Viewport` you set up in xref:03_Drawing_a_triangle/03_Drawing/01_Command_buffers.adoc[Command buffers] and give it a negative height:
