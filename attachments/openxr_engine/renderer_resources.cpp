...
bool Renderer::createDepthResources() {
  try {
    ...
    // Create depth image view (with layerCount=layers)
    depthImageView = createImageView(depthImage, depthFormat, vk::ImageAspectFlagBits::eDepth, 1, layers);
...
bool Renderer::createUniformBuffers(Entity* entity) {
  ...
  try {
    ...
    uint32_t numUbos = MAX_FRAMES_IN_FLIGHT; // Multiview uses one UBO per frame for both eyes
...
bool Renderer::createDescriptorSets(Entity* entity, EntityResources& res, const std::string& texturePath, bool usePBR) {
  ...
  try {
    ...
    uint32_t numSets = MAX_FRAMES_IN_FLIGHT; // Multiview uses one set per frame for both eyes
    std::vector<vk::DescriptorSetLayout> layouts(numSets, selectedLayout);
    vk::DescriptorSetAllocateInfo allocInfo{.descriptorPool = *descriptorPool, .descriptorSetCount = numSets, .pSetLayouts = layouts.data()};
    ...
    if (targetDescriptorSets.empty() || targetDescriptorSets.size() < numSets) {
    ...
    // Only initialize the current frame's descriptor set
    // In multiview, both eyes share the same descriptor set for the current frame
    {
      size_t i = static_cast<size_t>(currentFrame);
      ...
      if (usePBR) {
        ...
        lightBufferInfo = vk::DescriptorBufferInfo{.buffer = *lightStorageBuffers[i % MAX_FRAMES_IN_FLIGHT].buffer, .range = VK_WHOLE_SIZE};
        ...
        if ((i % MAX_FRAMES_IN_FLIGHT) < forwardPlusPerFrame.size()) {
          auto& f = forwardPlusPerFrame[i % MAX_FRAMES_IN_FLIGHT];
          if (!*f.tileHeaders) {
