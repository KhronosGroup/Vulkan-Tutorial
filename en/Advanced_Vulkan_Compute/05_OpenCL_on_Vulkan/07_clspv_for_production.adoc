...
Variable-length arrays are https://registry.khronos.org/OpenCL/specs/unified/html/OpenCL_C.html#_footnotedef_31[not supported in OpenCL C] — this is a spec restriction, not a `clspv` limitation.  `__local` array sizes must be fixed at compile time.  In practice this means using a `#define` or a specialisation constant for any local array dimension that you might otherwise want to pass as a kernel argument.
