            ...
            if (imageIndex >= 0) {
              ...
              if (loadedTextures.find(textureId) == loadedTextures.end()) {
                ...
                if (!image.image.empty()) {
                  ...
                  material->albedoTexturePath = baseTexturePath + image.uri;
                }
              }
            }
          }
...
bool ModelLoader::ParseGLTF(const std::string& filename, Model* model) {
  ...
  std::cout << "Parsing GLTF file: " << resolvedPath << " (original: " << filename << ")" << std::endl;
  ...
  // Load the GLTF file
  bool ret = false;
  if (resolvedPath.find(".glb") != std::string::npos) {
    ret = loader.LoadBinaryFromFile(&gltfModel, &err, &warn, resolvedPath);
  } else {
    ret = loader.LoadASCIIFromFile(&gltfModel, &err, &warn, resolvedPath);
  }
  ...
  if (!ret) {
    std::cerr << "Failed to parse GLTF file: " << resolvedPath << " (original: " << filename << ")" << std::endl;
    return false;
  }
