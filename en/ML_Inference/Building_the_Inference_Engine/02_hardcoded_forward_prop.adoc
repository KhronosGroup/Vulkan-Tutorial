...
The C++ code to read this is straightforward. You open the file, verify the magic number, read counts, read float arrays. See link:/attachments/ml_inference/weight_loader.hpp[weight_loader.hpp] for the complete implementation—it's about 50 lines total.
...
See link:/attachments/ml_inference/vulkan_mnist_inference.cpp[vulkan_mnist_inference.cpp] for the complete implementation. The core inference loop is about 150 lines, most of it Vulkan boilerplate.
...
You train a model in PyTorch or TensorFlow, then export the weights. The training script is in link:/attachments/ml_inference/train_mnist.py[train_mnist.py]. It:
