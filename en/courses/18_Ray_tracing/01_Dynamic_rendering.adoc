...
In dynamic rendering, we no longer create a `vk::RenderPass` or `vk::Framebuffer`; instead we begin rendering with `commandBuffer.beginRendering()`, specifying attachments on-the-fly. This makes our code more flexible (no need to predeclare subpasses) and is now the "modern" way to render in Vulkan.
