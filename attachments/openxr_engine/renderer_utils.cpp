...
QueueFamilyIndices Renderer::findQueueFamilies(const vk::raii::PhysicalDevice& device) {
  ...
  for (uint32_t i = 0; i < queueFamilies.size(); i++) {
    ...
    // In XR mode the surface is created after device selection, so the surface handle may be
    // VK_NULL_HANDLE here.  Guard the present-support query to avoid passing an invalid handle
    // to the driver (which crashes when the XR runtime intercepts surface creation at a layer
    // that sits outside the validation layer's view).
