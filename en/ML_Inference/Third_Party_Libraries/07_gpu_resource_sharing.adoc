...
[source,cpp]
----
VkPhysicalDeviceExternalMemoryHostPropertiesEXT externalMemoryProps = {};
externalMemoryProps.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_EXTERNAL_MEMORY_HOST_PROPERTIES_EXT;
...
VkExternalMemoryHandleTypeFlagsKHR supportedHandleTypes = 0;

#ifdef _WIN32
    supportedHandleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT_KHR;
#else
    supportedHandleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT_KHR;
#endif

// Query if this handle type is supported for images
VkPhysicalDeviceExternalImageFormatInfo externalImageFormatInfo = {};
externalImageFormatInfo.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_EXTERNAL_IMAGE_FORMAT_INFO;
externalImageFormatInfo.handleType = supportedHandleTypes;
...
VkResult result = vkGetPhysicalDeviceImageFormatProperties2(
    physicalDevice, &imageFormatInfo, &imageFormatProps2);

if (result == VK_SUCCESS) {
    // Check what operations are supported
    bool canExport = externalImageFormatProps.externalMemoryProperties.externalMemoryFeatures &
                     VK_EXTERNAL_MEMORY_FEATURE_EXPORTABLE_BIT_KHR;
    bool canImport = externalImageFormatProps.externalMemoryProperties.externalMemoryFeatures &
...
#else
    externalMemoryImageInfo.handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT_KHR;
#endif
...
exportInfo.handleTypes = externalMemoryImageInfo.handleTypes;
...
----
    ...
    // This handle can now be passed to DirectML, CUDA, or other APIs
#else
    ...
    // This file descriptor can now be passed to CUDA, OpenCL, or other APIs
#endif
----
...
*POSIX File Descriptors* - The standard mechanism. File descriptors are integers that reference kernel objects. When you export Vulkan memory as a file descriptor, you're getting a reference to the underlying kernel memory object. This works with CUDA, OpenCL, and most other GPU APIs on Linux.
...
ONNX Runtime's CUDA execution provider can import external memory using CUDA's external memory API:

[source,cpp]
----
#ifdef _WIN32
    // Windows: Import NT handle
    cudaExternalMemoryHandleDesc externalMemoryHandleDesc = {};
    externalMemoryHandleDesc.type = cudaExternalMemoryHandleTypeOpaqueWin32;
    externalMemoryHandleDesc.handle.win32.handle = win32Handle;
    externalMemoryHandleDesc.size = memorySize;

    cudaExternalMemory_t externalMemory;
    cudaImportExternalMemory(&externalMemory, &externalMemoryHandleDesc);
#else
    // Linux: Import file descriptor
    cudaExternalMemoryHandleDesc externalMemoryHandleDesc = {};
...
void* cudaPtr;
cudaExternalMemoryGetMappedBuffer(&cudaPtr, externalMemory, &bufferDesc);

// Now cudaPtr points to the same GPU memory as your Vulkan image
// You can pass this pointer to ONNX Runtime as input
----

Once you have the CUDA pointer, you can create an ONNX Runtime tensor that wraps it:

[source,cpp]
----
// Create ONNX Runtime tensor from CUDA memory
...
Ort::Value inputTensor = Ort::Value::CreateTensor(
    memoryInfo, cudaPtr, memorySize, shape.data(), shape.size(),
...
GPUs are asynchronous. When you submit a command buffer to Vulkan, it returns immediately—the GPU executes the commands later. Similarly, when you launch a CUDA kernel or DirectML operation, it returns immediately. Without synchronization, you have race conditions:
...
// Export the semaphore handle
    ...
    handleInfo.semaphore = renderCompleteSemaphore;
    handleInfo.handleType = VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_BIT;

    HANDLE semaphoreHandle;
    vkGetSemaphoreWin32HandleKHR(device, &handleInfo, &semaphoreHandle);
#else
    VkSemaphoreGetFdInfoKHR fdInfo = {};
    fdInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_GET_FD_INFO_KHR;
    fdInfo.semaphore = renderCompleteSemaphore;
    fdInfo.handleType = VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT;

    int semaphoreFd;
    vkGetSemaphoreFdKHR(device, &fdInfo, &semaphoreFd);
#endif
----

You then import this semaphore into your ML library (e.g., CUDA):

[source,cpp]
----
// Import semaphore into CUDA
...
#else
    semaphoreDesc.type = cudaExternalSemaphoreHandleTypeOpaqueFd;
    semaphoreDesc.handle.fd = semaphoreFd;
#endif

cudaExternalSemaphore_t cudaSemaphore;
cudaImportExternalSemaphore(&cudaSemaphore, &semaphoreDesc);
----

Now you can coordinate between Vulkan and CUDA:
...
// CUDA: Wait on semaphore, then run inference
cudaExternalSemaphoreWaitParams waitParams = {};
cudaWaitExternalSemaphoresAsync(&cudaSemaphore, &waitParams, 1, cudaStream);

// Launch CUDA kernels (or ONNX Runtime inference) on cudaStream
// They will wait for the semaphore before executing

// CUDA: Signal another semaphore when inference completes
cudaExternalSemaphoreSignalParams signalParams = {};
cudaSignalExternalSemaphoresAsync(&inferenceCompleteSemaphore, &signalParams, 1, cudaStream);

// Vulkan: Wait on inference complete semaphore before using results
VkSubmitInfo submitInfo2 = {};
submitInfo2.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
submitInfo2.waitSemaphoreCount = 1;
submitInfo2.pWaitSemaphores = &inferenceCompleteSemaphore;
VkPipelineStageFlags waitStage = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT;
submitInfo2.pWaitDstStageMask = &waitStage;
submitInfo2.commandBufferCount = 1;
submitInfo2.pCommandBuffers = &postProcessCommandBuffer;

vkQueueSubmit(graphicsQueue, 1, &submitInfo2, VK_NULL_HANDLE);
----

This creates a dependency chain: Vulkan renders → signals semaphore → CUDA waits → CUDA runs inference → signals semaphore → Vulkan waits → Vulkan uses results. All GPU-side, no CPU involvement, maximum performance.
...
Use validation layers aggressively. Enable Vulkan validation layers, CUDA's memory checker, and any debugging tools your ML library provides. Insert explicit synchronization (fences, cudaDeviceSynchronize) during debugging to isolate timing issues.
...
Each ML library has its own API for importing external memory—CUDA's external memory API for ONNX Runtime, AHardwareBuffer for TensorFlow Lite on Android, D3D12 resource sharing for DirectML. The concepts are similar across libraries: export from Vulkan, import into the ML library, synchronize with external semaphores.
