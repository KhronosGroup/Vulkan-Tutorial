...
[NOTE]
====
We covered PBR material theory and shader details earlier in xref:Building_a_Simple_Engine/Loading_Models/05_pbr_rendering.adoc[PBR Rendering], so we won't restate that here. This section focuses on the wiring: how material properties are packed into push constants and consumed by the draw call in this chapter's context.
====
...
xref:Building_a_Simple_Engine/Loading_Models/06_multiple_objects.adoc[Previous: Rendering Multiple Objects] | xref:Building_a_Simple_Engine/Loading_Models/08_animations.adoc[Next: Updating Animations]
