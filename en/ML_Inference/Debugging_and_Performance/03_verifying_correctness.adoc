...
The only way to debug a neural network is to compare it against a **Golden Reference**—a known-correct implementation that you trust implicitly. Typically, this is the original model running in PyTorch or ONNX Runtime (CPU).

The strategy is simple but rigorous:
1.  **Harvest**: Extract the output of every single layer from the Golden Reference.
2.  **Compare**: Run your Vulkan implementation and "dump" the output of every corresponding compute shader.
...
*   For a value of `1000.0`, a difference of `0.00001` is **impossible** to achieve due to how floating point precision works.
...
== Harvesting the Truth: A Python Script

To verify your model, you need a script that acts as your "Ground Truth Generator." Here is a substantive template for harvesting layer-by-layer activations from PyTorch.
...
# We want to save the output of every layer during a single forward pass
activations = {}
def get_activation(name):
    def hook(model, input, output):
        # We detach from the graph and move to CPU before converting to Numpy
        activations[name] = output.detach().cpu().numpy()
    return hook

# Register hooks for the layers we want to verify
# Note: You can find these names by printing 'model' or using Netron
model.features[0].register_forward_hook(get_activation('conv_1'))
model.features[1].register_forward_hook(get_activation('block_1'))
model.features[18].register_forward_hook(get_activation('last_conv'))

# 3. Run a deterministic input
# Use a fixed random seed or a specific image
torch.manual_seed(42)
input_tensor = torch.randn(1, 3, 224, 224)
output = model(input_tensor)

# 4. Save to disk (Numpy format is easy for C++ to read)
np.save("ref_input.npy", input_tensor.numpy())
for name, tensor in activations.items():
    np.save(f"ref_{name}.npy", tensor)
    print(f"Saved {name}: mean={tensor.mean():.4f}, std={tensor.std():.4f}")
----
...
void verifyBuffer(vk::Buffer deviceBuffer, vk::DeviceSize size, const std::string& name) {
    ...
    float* mappedData = static_cast<float*>(stagingMemory.mapMemory(0, size));
    compareWithRef(mappedData, name);
    stagingMemory.unmapMemory();
}
----

== Case Study: The "Associativity" Trap

One of the most confusing parts of numerical verification is that **floating-point addition is not associative**.
[latexmath]
++++
(a + b) + c \neq a + (b + c)
++++

In a Vulkan shader, you might use **Shared Memory** to sum up values for a dot product (a reduction). Depending on your workgroup size, you might sum them in a different order than PyTorch does on the CPU.

*   **The Result**: Even with a "perfect" implementation, your results might differ by latexmath:[10^{-5}] or latexmath:[10^{-6}].
*   **The Lesson**: Do not aim for 0.0 error. Aim for "Stable" error. If your MAE is consistently latexmath:[10^{-6}], your logic is likely correct. If it jumps to latexmath:[10^{-2}], you have a bug.
...
== The Comparison Table: Interpreting Error Patterns
...
A professional ML pipeline runs verification on every commit. You should add a `--verify` flag to your C{pp} application that:
1.  Loads the `.npy` files.
2.  Runs one inference pass.
3.  Calculates MAE for each layer.
4.  Exits with code `0` only if all layers pass.

[source,cmake]
----
# Example CTest integration
add_test(NAME Vulkan_ML_Accuracy_Test
         COMMAND my_inference_app --verify --ref_dir ./golden_references)
----
...
*   **Harvest** ground truth from a trusted framework.
...
xref:02_debugging_vulkan_compute.adoc[Previous: Debugging Vulkan Compute] | xref:04_performance_profiling.adoc[Next: Performance Profiling]
