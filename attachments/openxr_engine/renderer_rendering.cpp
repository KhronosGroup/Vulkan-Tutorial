...
bool Renderer::createSwapChain() {
  try {
    if (xrMode) {
      ...
      // Query swap chain support (though XR often dictates this)
...
void Renderer::updateUniformBufferInternal(uint32_t currentImage, Entity* entity, EntityResources* entityRes, CameraComponent* camera, UniformBufferObject& ubo) {
  ...
  if (ubo.reflectionPass == 1) {
    finalUbo.views[0] = ubo.views[0];
    finalUbo.projs[0] = ubo.projs[0];
    finalUbo.viewProjections[0] = ubo.projs[0] * ubo.views[0];
...
void Renderer::drawRenderJob(const vk::raii::CommandBuffer& cmd, const RenderJob& job, uint32_t currentFrame, uint32_t eye, bool transparent) {
  EntityResources* entityRes = job.entityRes;
  MeshResources* meshRes = job.meshRes;
  MeshComponent* meshComponent = job.meshComp;

  // 1. Determine descriptor set index
  ...
  cmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                         *pbrPipelineLayout,
                         0,
                         *entityRes->pbrDescriptorSets[setIndex],
                         nullptr);

  // 5. Push material properties
  pushMaterialProperties(*cmd, entityRes->cachedMaterialProps);
...
void Renderer::Render(const std::vector<Entity *>& entities, CameraComponent* camera, ImGuiSystem* imguiSystem, XrTime predictedTime) {
  ...
  if (fenceResult != vk::Result::eSuccess) {
    std::cerr << "Error: Failed to wait for in-flight fence (XR): " << vk::to_string(fenceResult) << std::endl;
  }
  ...
  // we can use a combined frustum or just render everything for both eyes.
  std::vector<RenderJob> opaqueJobs;
  std::vector<RenderJob> transparentJobs;
  // ... Simplified: just use all active entities for both eyes for now ...
  ...
  // 1. Acquire and Wait for OpenXR swapchain image
  ...
  for (uint32_t eye = 0; eye < 2; ++eye) {
    glm::mat4 view = camera ? camera->GetViewMatrix(eye) : glm::mat4(1.0f);
    glm::mat4 proj = camera ? camera->GetProjectionMatrix(eye) : glm::mat4(1.0f);
    proj[1][1] *= -1.0f; // Vulkan Y-flip
    frameUboTemplate.views[eye] = view;
    frameUboTemplate.projs[eye] = proj;
    frameUboTemplate.viewProjections[eye] = proj * view;
    frameUboTemplate.camPoses[eye] = glm::vec4(xrContext.getEyePosition(eye), 1.0f);
  }

  // 4. Begin Rendering (Single-Pass Multiview)
  vk::RenderingAttachmentInfo colorAtt{
  ...
  // Set Viewport and Scissor (shared for both eyes in multiview)
  cmd.setViewport(0, xrContext.getViewport(0));
