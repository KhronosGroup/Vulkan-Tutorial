...
bool Renderer::createTextureImage(const std::string &texturePath_, TextureResources &resources)
{
  ...
  std::vector<vk::BufferImageCopy> copyRegions;
   ...
   for (uint32_t level = 0; level < mipLevels; ++level)
   {
       ...
       copyRegions.push_back({.bufferOffset      = static_cast<vk::DeviceSize>(levelOffset),
                              .bufferRowLength   = 0,
                              .bufferImageHeight = 0,
                              .imageSubresource  = {
  ...
  // If KTX2 provided mip levels, use them as-is. Otherwise, optionally generate for uncompressed RGBA.
  bool needGenerateMips = false;
  if (!isKtx2)
  {
      if (textureFormat == vk::Format::eR8G8B8A8Srgb || textureFormat == vk::Format::eR8G8B8A8Unorm)
      {
  ...
  // usage flags: only require TRANSFER_SRC when we intend to generate mips on GPU
  ...
  // Generate mip chain if requested (only for uncompressed RGBA textures not coming from KTX2)
  if (!isKtx2 && needGenerateMips && (textureFormat == vk::Format::eR8G8B8A8Srgb || textureFormat == vk::Format::eR8G8B8A8Unorm))
  {
