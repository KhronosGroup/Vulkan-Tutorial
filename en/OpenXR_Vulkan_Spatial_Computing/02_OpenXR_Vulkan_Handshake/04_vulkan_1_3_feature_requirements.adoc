...
In this section, we'll look at the three mandatory "pillars" of Vulkan 1.3 that our spatial pipeline depends on.
...
[source,cpp]
----
// Enabling mandatory features for our spatial pipeline, including new Vulkan 1.3 capabilities
...
// Ensure we are explicitly asking for Vulkan 1.3 core features if needed
// Note: In Vulkan 1.3, many of these are enabled by default if the version is supported,
