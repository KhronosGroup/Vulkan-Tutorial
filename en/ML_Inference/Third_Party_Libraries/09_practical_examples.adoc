...
Our first example demonstrates the most common desktop scenario: running object detection on rendered frames using ONNX Runtime with CUDA execution provider, sharing GPU memory between Vulkan and CUDA.
...
Before we dive into code, let's understand how the pieces fit together. Think of this as a relay race where Vulkan and CUDA pass the baton (GPU memory) back and forth, but instead of runners physically handing off a baton, they use semaphores to signal "I'm done, you can go now."

Here's the flow:

Vulkan renders your 3D scene to a special texture—one that's allocated with external memory support so CUDA can access it. When rendering finishes, Vulkan signals a semaphore. CUDA, which has been waiting on that semaphore, wakes up and tells ONNX Runtime "the data is ready." ONNX Runtime runs inference directly on that shared texture—no copying to CPU, no uploading back to GPU. When inference completes, CUDA signals another semaphore. Vulkan waits on that semaphore before using the results.
...
The foundation of this entire approach is creating a Vulkan image that CUDA can access. This isn't your typical Vulkan image—it needs special flags that tell Vulkan "other APIs will need to access this memory." On Windows, we'll export a Win32 handle; on Linux, we'll export a file descriptor. Both serve the same purpose: giving CUDA a way to reference the same physical GPU memory.
...
=== Step 2: Importing into CUDA

Now that we have a Vulkan image with an exported handle, we need to tell CUDA about it. CUDA has its own way of thinking about memory—it uses "external memory" objects that wrap handles from other APIs. We'll import the handle, map it to a CUDA array (CUDA's equivalent of a texture), and create a surface object that makes it easy to read and write pixel data.

Think of this step as translating between Vulkan's language and CUDA's language. Both are talking about the same physical memory, but they use different vocabulary to describe it.
...
public:
    ...
    void importFromVulkan(const SharedRenderTarget& vulkanTarget,
        ...
        memHandleDesc.handle.win32.handle = vulkanTarget.memoryHandle;
#else
        memHandleDesc.type = cudaExternalMemoryHandleTypeOpaqueFd;
        memHandleDesc.handle.fd = vulkanTarget.memoryFd;
#endif
        memHandleDesc.size = width * height * 4; // RGBA
        ...
        cudaExternalMemoryGetMappedMipmappedArray(&mipmappedArray, externalMemory, &arrayDesc);
        cudaGetMipmappedArrayLevel(&array, mipmappedArray, 0);

        // Create surface object for easy access
...
Sharing memory is only half the battle. We also need to coordinate timing—Vulkan can't start rendering to the texture while CUDA is reading from it, and CUDA can't start inference until Vulkan finishes rendering. This is where semaphores come in.

Just like we exported memory handles, we'll export semaphore handles that both Vulkan and CUDA can use. Vulkan signals a semaphore when it's done rendering, CUDA waits on that semaphore before starting inference, then CUDA signals another semaphore when inference completes, and Vulkan waits on that before using the results. It's a carefully choreographed dance that keeps both APIs from stepping on each other's toes.
...
public:
    ...
    void create(VkDevice device) {
        ...
        cudaImportExternalSemaphore(&cudaSemaphore, &semHandleDesc);
    }
};
----
...
With shared memory and synchronization in place, we're ready for the main event: running inference. ONNX Runtime needs to be configured to use CUDA as its execution provider, and we need to tell it to use our shared memory for input. The key insight here is that ONNX Runtime can create tensors directly from CUDA memory—we don't need to copy data into ONNX Runtime's own buffers.
...
public:
    ObjectDetector(const std::string& modelPath)
        ...
        // Configure ONNX Runtime to use CUDA
    ...
    void runInference(const CUDASharedMemory& sharedMem,
        ...
        cudaWaitExternalSemaphoresAsync(&renderComplete.cudaSemaphore,
                                       &waitParams, 1, stream);
        ...
        cudaSignalExternalSemaphoresAsync(&inferenceComplete.cudaSemaphore,
                                         &signalParams, 1, stream);
    }
};
----
...
Now let's see how all these pieces work together in a real render loop. Each frame, we'll render with Vulkan, signal that rendering is complete, let CUDA run inference on the shared texture, wait for inference to complete, and then use the results. The beauty of this approach is that once everything is set up, the per-frame overhead is minimal—just signaling and waiting on semaphores, which are GPU-side operations.
...
void renderLoop() {
    ...
    CUDASharedMemory cudaMemory;
    cudaMemory.importFromVulkan(renderTarget, 1920, 1080);
    ...
    while (!shouldClose()) {
        ...
        // CUDA/ONNX: Run inference (async)
        detector.runInference(cudaMemory, renderComplete, inferenceComplete,
                            1920, 1080);
...
public:
    PipelinedInference(const std::string& modelPath)
        ...
        for (int i = 0; i < FRAMES_IN_FLIGHT; i++) {
            ...
            frames[i].cudaMemory.importFromVulkan(frames[i].renderTarget, 1920, 1080);
    ...
    void processFrame() {
        ...
        std::thread([this, &frame]() {
            detector.runInference(frame.cudaMemory,
...
*Solution*: Use validation layers aggressively. Enable Vulkan validation layers and CUDA's memory checker. Insert explicit synchronization (fences, cudaDeviceSynchronize) during debugging to isolate timing issues.
...
*Cause*: Not properly releasing external memory handles, semaphores, or CUDA resources.
...
These practical examples demonstrate real-world integration patterns for ML inference with Vulkan. The desktop example shows ONNX Runtime with CUDA and external memory sharing. The mobile example shows TensorFlow Lite on Android with AHardwareBuffer. The pipelined example shows how to achieve real-time performance through parallelism.
