  ...
  if (renderer) {
    ...
    if (modelLoading) {
      ...
      if (ImGui::Begin("##LoadingOverlay", nullptr, flags)) {
        ...
        // Global monotonic progress for the main loading bar to avoid resets
        float frac = renderer->GetGlobalLoadingProgress();
        auto phase = renderer->GetLoadingPhase();

        ImGui::ProgressBar(frac, ImVec2(barWidth, 0.0f));
        ImGui::Dummy(ImVec2(0.0f, 10.0f));
        ImGui::SetCursorPosX(barX);
        ImGui::Text("Loading: %s (%.1f%%)", renderer->GetLoadingPhaseName(), frac * 100.0f);
...
void ImGuiSystem::EndFrameWithoutRendering() {
  if (!initialized || frameAlreadyRendered) {
    return;
  }
  ImGui::EndFrame();
