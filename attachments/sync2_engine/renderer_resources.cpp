...
bool Renderer::createTextureImage(const std::string& texturePath_, TextureResources& resources) {
  try {
    ...
    std::string resolvedPath = ResolvePath(textureId);
...
bool Renderer::createMeshResources(MeshComponent* meshComponent, bool deferUpload) {
  ensureThreadLocalVulkanInit();
  try {
    // If resources already exist, check if we need to flush staging buffers
    {
      std::shared_lock<std::shared_mutex> lock(meshResourcesMutex);
      ...
      if (it != meshResources.end()) {
        if (!deferUpload) {
          ...
          if ((res.vertexBufferSizeBytes > 0 && !!*res.stagingVertexBuffer && !!*res.vertexBuffer) ||
            ...
            // Need unique lock to modify res (and meshResources values)
            lock.unlock();
            std::unique_lock<std::shared_mutex> uniqueLock(meshResourcesMutex);
            auto itUnique = meshResources.find(meshComponent);
            if (itUnique != meshResources.end()) {
              MeshResources& resU = itUnique->second;
              if (resU.vertexBufferSizeBytes > 0 && !!*resU.stagingVertexBuffer && !!*resU.vertexBuffer) {
                copyBuffer(resU.stagingVertexBuffer, resU.vertexBuffer, resU.vertexBufferSizeBytes);
                resU.stagingVertexBuffer = vk::raii::Buffer(nullptr);
                resU.stagingVertexBufferAllocation = nullptr;
                resU.vertexBufferSizeBytes = 0;
              }
              if (resU.indexBufferSizeBytes > 0 && !!*resU.stagingIndexBuffer && !!*resU.indexBuffer) {
                copyBuffer(resU.stagingIndexBuffer, resU.indexBuffer, resU.indexBufferSizeBytes);
                resU.stagingIndexBuffer = vk::raii::Buffer(nullptr);
                resU.stagingIndexBufferAllocation = nullptr;
                resU.indexBufferSizeBytes = 0;
              }
            }
          }
        }
    ...
    // --- 1. Create and fill per-mesh staging buffers using memory pool ---
    vk::DeviceSize vertexBufferSize = sizeof(vertices[0]) * vertices.size();
    KickWatchdog();
    auto [stagingVertexBuffer, stagingVertexBufferAllocation] = createBufferPooled(
      vertexBufferSize,
      vk::BufferUsageFlagBits::eTransferSrc,
      vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);

    if (!stagingVertexBufferAllocation) {
      throw std::runtime_error("Failed to allocate staging vertex buffer from memory pool");
    }
    void* vertexData = stagingVertexBufferAllocation->mappedPtr;
    if (vertexData) {
    ...
    auto [stagingIndexBuffer, stagingIndexBufferAllocation] = createBufferPooled(
      indexBufferSize,
      vk::BufferUsageFlagBits::eTransferSrc,
      vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent);

    if (!stagingIndexBufferAllocation) {
      throw std::runtime_error("Failed to allocate staging index buffer from memory pool");
    }
    void* indexData = stagingIndexBufferAllocation->mappedPtr;
    if (indexData) {
    ...
    if (deferUpload) {
      ...
      resources.stagingVertexBufferAllocation = std::move(stagingVertexBufferAllocation);
      resources.vertexBufferSizeBytes = vertexBufferSize;

      resources.stagingIndexBuffer = std::move(stagingIndexBuffer);
      resources.stagingIndexBufferAllocation = std::move(stagingIndexBufferAllocation);
    ...
    // Add to mesh resources map with unique lock
    {
      std::unique_lock<std::shared_mutex> lock(meshResourcesMutex);
...
bool Renderer::createUniformBuffers(Entity* entity) {
  ...
  try {
    ...
    // Check if entity resources already exist with shared lock
    {
      std::shared_lock<std::shared_mutex> lock(entityResourcesMutex);
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      ...
      if (!bufferAllocation) {
        throw std::runtime_error("Failed to allocate uniform buffer from memory pool");
      }
    ...
    resources.lastUpdatedFrameBasic.assign(MAX_FRAMES_IN_FLIGHT, 0xFFFFFFFFFFFFFFFFULL);
    resources.lastUpdatedFramePBR.assign(MAX_FRAMES_IN_FLIGHT, 0xFFFFFFFFFFFFFFFFULL);
    ...
    if (meshComponent) {
      ...
      if (!instanceBufferAllocation) {
        throw std::runtime_error("Failed to allocate instance buffer from memory pool");
      }
    ...
    // Add to entity resources map with unique lock
    {
      std::unique_lock<std::shared_mutex> lock(entityResourcesMutex);
...
bool Renderer::createDescriptorSets(Entity* entity, EntityResources& res, const std::string& texturePath, bool usePBR) {
  ...
  try {
    ...
    for (size_t i = startIndex; i < endIndex; i++) {
      // Optimization: skip if already updated for this frame slot since the entity was last marked dirty
      auto& lastUpdatedVec = usePBR ? res.lastUpdatedFramePBR : res.lastUpdatedFrameBasic;
      if (lastUpdatedVec[i] != 0xFFFFFFFFFFFFFFFFULL) {
        continue;
      }

      // Update the frame tracker BEFORE we might potentially return on error,
      // though typically we'd only want to mark success.
      // We mark it now so we don't spam errors every frame if it fails once.
      lastUpdatedVec[i] = totalFrameCount.load(std::memory_order_relaxed);
      ...
      if (usePBR) {
        ...
        auto meshComponent = entity->GetComponent<MeshComponent>();
        if (meshComponent) {
          std::array<std::string, 5> pbrTexturePaths;
          {
            const std::string legacyPath = meshComponent->GetTexturePath();
            pbrTexturePaths[0] = (!meshComponent->GetBaseColorTexturePath().empty()) ? meshComponent->GetBaseColorTexturePath() : (!legacyPath.empty() ? legacyPath : SHARED_DEFAULT_ALBEDO_ID);
            pbrTexturePaths[1] = (!meshComponent->GetMetallicRoughnessTexturePath().empty()) ? meshComponent->GetMetallicRoughnessTexturePath() : SHARED_DEFAULT_METALLIC_ROUGHNESS_ID;
            pbrTexturePaths[2] = (!meshComponent->GetNormalTexturePath().empty()) ? meshComponent->GetNormalTexturePath() : SHARED_DEFAULT_NORMAL_ID;
            pbrTexturePaths[3] = (!meshComponent->GetOcclusionTexturePath().empty()) ? meshComponent->GetOcclusionTexturePath() : SHARED_DEFAULT_OCCLUSION_ID;
            pbrTexturePaths[4] = (!meshComponent->GetEmissiveTexturePath().empty()) ? meshComponent->GetEmissiveTexturePath() : SHARED_DEFAULT_EMISSIVE_ID;
          }
  ...
  try {
    ...
    // Initialize per-frame UBO and image binding write flags
    {
      ...
      if (it != entityResources.end()) {
        ...
        it->second.lastUpdatedFrameBasic.assign(MAX_FRAMES_IN_FLIGHT, 0xFFFFFFFFFFFFFFFFULL);
        it->second.lastUpdatedFramePBR.assign(MAX_FRAMES_IN_FLIGHT, 0xFFFFFFFFFFFFFFFFULL);
      }
    }
...
bool Renderer::preAllocateEntityResourcesBatch(const std::vector<Entity *>& entities) {
  ...
  try {
    ...
    for (Entity* entity : entities) {
      ...
      // Kick watchdog only occasionally during heavy mesh resource creation
      if (++processedMeshes % 20 == 0) {
        KickWatchdog();
      }
		...
		if (!meshesNeedingUpload.empty())
    {
      ...
      if (flushUploadsNow) {
        // We used to call ProcessPendingMeshUploads() here, but it's redundant and slow
        // because Render() calls it once per frame already.
      }
    }
    ...
    for (Entity* entity : entities) {
      ...
      // Kick watchdog only occasionally during heavy resource creation
      if (++processedResources % 20 == 0) {
        KickWatchdog();
      }
      ...
      // Optimization: Only create the descriptor sets that will actually be used.
      // In the Bistro scene and most modern GLTF loads, we use PBR by default.
      // We skip basic descriptor sets unless the entity is explicitly non-PBR.
      bool usePBR = true;
...
void Renderer::ProcessPendingEntityPreallocations() {
  ...
  // Suppress watchdog during heavy preallocation batches
  ScopedWatchdogSuppression watchdogGuard(this);
  ...
  std::vector<Entity *> toRecreateInstances; {
    ...
    // Time-slice preallocations to avoid blocking the render thread for huge scenes.
    // Process a larger chunk while loading to speed up startup, and a smaller chunk
    // during normal gameplay to maintain high FPS.
    const size_t preallocChunkSize = IsLoading() ? 100 : 1;
    if (!pendingEntityPrealloc.empty()) {
      size_t count = std::min(pendingEntityPrealloc.size(), preallocChunkSize);
      toPreallocate.insert(toPreallocate.end(), pendingEntityPrealloc.begin(), pendingEntityPrealloc.begin() + count);
      pendingEntityPrealloc.erase(pendingEntityPrealloc.begin(), pendingEntityPrealloc.begin() + count);
    }

    // Always process all instance recreations as they are usually fewer and more critical
  ...
  if (!batch.empty()) {
    ...
    if (!preAllocateEntityResourcesBatch(batch)) {
      std::cerr << "Warning: batch entity GPU preallocation failed; some entities may be missing or retried later" << std::endl;
      // Note: we don't put them back at the head of the queue to avoid infinite loops on invalid data,
      // but they remain in the scene and might be retried if their components are refreshed.
      // For now, most failures are fatal per-entity but shouldn't crash the renderer.
    }
    CompleteLoadingWorkItems(batch.size());
  }
  ...
  try {
    ...
    if (!!*uploadsTimeline) {
      ...
      while (true) {
        ...
        uint64_t batchValue = 0; {
          std::lock_guard<std::mutex> lk(inFlightMeshUploadsMutex);
          if (inFlightMeshUploads.empty())
            break;
          if (inFlightMeshUploads.front().signalValue == 0)
            break;
          batchValue = inFlightMeshUploads.front().signalValue;

          // Use waitSemaphores with 0 timeout to poll for completion
          vk::Result waitResult = vk::Result::eTimeout;
          uint64_t currentCounter = 0;
          try {
            currentCounter = uploadsTimeline.getCounterValue();
            if (currentCounter >= batchValue) {
               waitResult = vk::Result::eSuccess;
            } else {
               // Also try polling with 0 timeout just in case getCounterValue is stale
               vk::SemaphoreWaitInfo waitInfo{
                 .semaphoreCount = 1,
                 .pSemaphores = &*uploadsTimeline,
                 .pValues = &batchValue
               };
               waitResult = device.waitSemaphores(waitInfo, 0);
            }
          } catch (...) {}
          ...
          // Now safe to retire the batch
          completedBatch = std::move(inFlightMeshUploads.front());
          inFlightMeshUploads.pop_front();

          // Double check synchronization: ensure the semaphore counter is indeed at least batchValue
          currentCounter = uploadsTimeline.getCounterValue();
          if (currentCounter < batchValue) {
             std::cerr << "CRITICAL: waitSemaphores returned Success but getCounterValue is still "
                       << currentCounter << " (expected >= " << batchValue << ")" << std::endl;
             // Push it back and break? No, if waitSemaphores lied, we are in trouble anyway.
          }
        }

        if (completedBatch.commandPool) {
          // Lock meshResources exclusively to modify staging buffer states
          std::unique_lock<std::shared_mutex> meshResourcesLock(meshResourcesMutex);
          ...
          for (auto* meshComponent : completedBatch.meshes) {
            ...
            if (it == meshResources.end())
              continue;
            MeshResources& res = it->second;
            // Accessing members of res (offset 8 for vertexBufferAllocation unique_ptr)
            res.stagingVertexBuffer = vk::raii::Buffer(nullptr);
            res.stagingVertexBufferAllocation = nullptr;
            res.vertexBufferSizeBytes = 0;
            res.stagingIndexBuffer = vk::raii::Buffer(nullptr);
            res.stagingIndexBufferAllocation = nullptr;
            res.indexBufferSizeBytes = 0;
          }
          anyCompleted = true;

          // Explicitly destroy command buffers and pool while we are here,
          // though RAII will do it when completedBatch goes out of scope.
          // This allows us to catch any exceptions during destruction.
          try {
             completedBatch.commandBuffers = nullptr;
             completedBatch.commandPool = nullptr;
          } catch (const std::exception& ex) {
             std::cerr << "Exception during batch retirement: " << ex.what() << std::endl;
          }
        }
      }

      if (anyCompleted && !IsLoading()) {
    ...
    // Record copies on GRAPHICS queue
    vk::CommandPoolCreateInfo poolInfo{
      .flags = vk::CommandPoolCreateFlagBits::eTransient | vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
      .queueFamilyIndex = queueFamilyIndices.graphicsFamily.value()
    };

    // Prefer async submission via the uploads timeline semaphore.
    // ONLY force synchronous if AS build is ALREADY requested AND we have no timeline.
    // Doing synchronous uploads during loading hangs the main thread for large scenes.
    const bool forceSynchronous = asBuildRequested.load(std::memory_order_relaxed) &&
                                  (!*uploadsTimeline || *uploadsTimeline == vk::Semaphore{});
    ...
    if (canSignalTimeline) {
      ...
      for (auto* meshComponent : needsCopy) {
        ...
        MeshResources* pRes = nullptr; {
          std::shared_lock<std::shared_mutex> meshLock(meshResourcesMutex);
          auto it = meshResources.find(meshComponent);
          if (it != meshResources.end()) pRes = &it->second;
        }
        if (!pRes) continue;
        MeshResources& res = *pRes;
        if (res.vertexBufferSizeBytes > 0 && !!*res.stagingVertexBuffer && !!*res.vertexBuffer) {
      ...
      SubmitToQueue2(*graphicsQueue, *cb, true, &signalVal, nullptr);

      InFlightMeshUploadBatch batch;
      batch.signalValue = signalVal;
      ...
      // Fallback: submit and wait (limit batch size to avoid long frame hangs)
      const size_t batchLimit = 50;
      for (size_t i = 0; i < needsCopy.size(); i += batchLimit) {
        size_t end = std::min(i + batchLimit, needsCopy.size());
        std::vector<MeshComponent*> batch(needsCopy.begin() + i, needsCopy.begin() + end);
        ...
        SubmitToQueue2(*graphicsQueue, *cb, false, nullptr, *fence);
      ...
      // Rebuild AS after uploads only if we are in Play state to avoid loading stutter.
      // Initial AS build is triggered separately when preallocation finishes.
      if (!IsLoading()) {
        asDevOverrideAllowRebuild = true;
        RequestAccelerationStructureBuild("uploads completed");
      }
    ...
    std::cerr << "CRITICAL: ProcessPendingMeshUploads error: " << e.what() << std::endl;
  }
}
...
std::pair<vk::raii::Buffer, vk::raii::DeviceMemory> Renderer::createBuffer(
  ...
  try {
    ...
    // Add allocation flags if requested (e.g. for buffers needing device address)
...
void Renderer::copyBuffer(vk::raii::Buffer& srcBuffer, vk::raii::Buffer& dstBuffer, vk::DeviceSize size) {
  ...
  try {
    ...
    vk::raii::Fence fence(device, vk::FenceCreateInfo{});
    SubmitToQueue2(*transferQueue, *commandBuffer, false, nullptr, *fence);
    (void) waitForFencesSafe(*fence, VK_TRUE);
  } catch (const std::exception& e) {
...
void Renderer::transitionImageLayout(vk::Image image, vk::Format format, vk::ImageLayout oldLayout, vk::ImageLayout newLayout, uint32_t mipLevels) {
  ...
  try {
    ...
    vk::raii::Fence fence(device, vk::FenceCreateInfo{});
    vk::CommandBufferSubmitInfo cmdInfo{.commandBuffer = *commandBuffer};
    ...
    vk::SubmitInfo2 submit2{
      ...
      .signalSemaphoreInfoCount = 0, // Synchronous wait below; no timeline signal needed
      .pSignalSemaphoreInfos = nullptr
    };
    Submit2(*graphicsQueue, submit2, *fence);
    (void) waitForFencesSafe(*fence, VK_TRUE);
  } catch (const std::exception& e) {
...
std::future<bool> Renderer::LoadTextureAsync(const std::string& texturePath, bool critical) {
  ...
  // Transition UI phase to Textures on the very first scheduled job so the overlay
  // doesn’t sit at 0% before any work is actually enqueued.
  if (textureTasksScheduled.load(std::memory_order_relaxed) == 0u) {
...
void Renderer::WaitForAllTextureTasks() {
  ...
  auto start = std::chrono::steady_clock::now();
  uint32_t lastCompleted = 0xFFFFFFFF;
  for (;;) {
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(now - start).count() > 60) {
      std::cerr << "WARNING: WaitForAllTextureTasks timed out after 60s! (" << completed << "/" << scheduled << ")" << std::endl;
      break;
    }
          ...
          try {
            ...
            for (auto& job : memJobs) {
              try {
                ...
                // Enable Host Transfer if supported (Vulkan 1.4)
                usage |= vk::ImageUsageFlagBits::eHostTransfer;

                auto [image, imageAlloc] = createImagePooled(job.width, job.height, texFormat, vk::ImageTiling::eOptimal, usage, vk::MemoryPropertyFlagBits::eDeviceLocal, 1, differentFamilies ? vk::SharingMode::eConcurrent : vk::SharingMode::eExclusive, families);
            ...
            if (!items.empty()) {
              // MODERN: Use Vulkan 1.4 Host Image Copies (Direct CPU-to-Image Access)
              // This completely avoids command buffer recording, pools, and fence waits!
              for (auto& it : items) {
                try {
                  // Transition undefined -> General using Host transition
                  vk::HostImageLayoutTransitionInfoEXT toHostCopy{
                    .image = *it.image,
                    .oldLayout = vk::ImageLayout::eUndefined,
                    .newLayout = vk::ImageLayout::eGeneral,
                    .subresourceRange = {.aspectMask = vk::ImageAspectFlagBits::eColor, .baseMipLevel = 0, .levelCount = 1, .baseArrayLayer = 0, .layerCount = 1}
                  };
                  device.transitionImageLayoutEXT(toHostCopy);

                  // Copy from host pointer directly to image
                  vk::MemoryToImageCopyEXT region{
                    .pHostPointer = it.rgba.data(),
                    .imageSubresource = {.aspectMask = vk::ImageAspectFlagBits::eColor, .mipLevel = 0, .baseArrayLayer = 0, .layerCount = 1},
                    .imageOffset = {0, 0, 0},
                    .imageExtent = {it.w, it.h, 1}
                  };
                  ...
                  // Final transition to shader-read for engine use
                  vk::HostImageLayoutTransitionInfoEXT toShaderRead{
                    .image = *it.image,
                    .oldLayout = vk::ImageLayout::eGeneral,
                    .newLayout = vk::ImageLayout::eShaderReadOnlyOptimal,
                    .subresourceRange = {.aspectMask = vk::ImageAspectFlagBits::eColor, .baseMipLevel = 0, .levelCount = 1, .baseArrayLayer = 0, .layerCount = 1}
                  };
                  device.transitionImageLayoutEXT(toShaderRead);
                  it.success = true;
                } catch (const std::exception& e) {
                  std::cerr << "HostImageCopy failed for '" << it.id << "': " << e.what() << std::endl;
                  it.success = false;
                  continue;
                }
              }
              ...
              for (auto& it : items) {
                ...
                // Note: textureTasksCompleted is incremented by OnTextureUploaded (if success)
                // or we increment it here manually if it failed.
                if (!it.success) {
...
void Renderer::OnTextureUploaded(const std::string& textureId) {
  ...
  // Increment completed tasks for progress tracking
  textureTasksCompleted.fetch_add(1, std::memory_order_relaxed);
...
void Renderer::MarkEntityDescriptorsDirty(Entity* entity) {
  ...
  // Also reset the last updated frame counters to force an update next time it's rendered
...
void Renderer::ProcessPendingTextureJobs(uint32_t maxJobs,
  ...
  // If the background uploads worker is running, it normally handles draining jobs.
  // However, to guarantee forward progress (and avoid UI stalls if a worker is backlogged),
  // we still drain a small bounded number of jobs on the render thread.
...
void Renderer::uploadImageFromStaging(vk::Buffer staging,
  ...
  try {
    ...
    // Choose a queue family for uploads: prefer dedicated TRANSFER if available, else use GRAPHICS
    bool hasTransferFamily = queueFamilyIndices.transferFamily.has_value();
    uint32_t uploadQueueFamily = hasTransferFamily ? queueFamilyIndices.transferFamily.value() : queueFamilyIndices.graphicsFamily.value();
    bool useTransferQueue = hasTransferFamily && !!*transferQueue;

    // Use a temporary transient command pool for the chosen queue family
    vk::CommandPoolCreateInfo poolInfo{
      .flags = vk::CommandPoolCreateFlagBits::eTransient | vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
    ...
    vk::raii::Fence fence(device, vk::FenceCreateInfo{});
    bool canSignalTimeline = false; // Synchronous wait below; no timeline signal needed
    uint64_t signalValue = 0;
    SubmitToQueue2(useTransferQueue ? *transferQueue : *graphicsQueue, *cb, canSignalTimeline, &signalValue, *fence);
    (void) waitForFencesSafe(*fence, VK_TRUE);
...
void Renderer::generateMipmaps(vk::Image image,
  ...
  vk::raii::Fence fence(device, vk::FenceCreateInfo{});
  bool canSignalTimeline = false; // Synchronous wait below; no timeline signal needed
  SubmitToQueue2(*graphicsQueue, *cb, canSignalTimeline, nullptr, *fence);
  (void) waitForFencesSafe(*fence, VK_TRUE);
}
