...
[source,multilang,c++,c]
.Recreating the swap chain after presentation
----
// START c++
...
Recall from the xref:03_Drawing_a_triangle/03_Drawing/03_Frames_in_flight.adoc[previous chapter] that the beginning of `drawFrame` waits for and then immediately resets the current frame's fence:
...
[source,multilang,c++,c]
.Deferring the fence reset until work is actually submitted
----
// START c++
auto fenceResult = device.waitForFences(*inFlightFences[frameIndex], vk::True, UINT64_MAX);
if (fenceResult != vk::Result::eSuccess)
...
[source,multilang,c++,c]
.recreateSwapChain, handling minimization
----
// START c++
void recreateSwapChain() {
