...
A logit of `88.0` is common. But latexmath:[e^{88}] is roughly `1.6 \times 10^{38}`—the absolute limit of a 32-bit float. A logit of `90.0` will overflow and return `Infinity`, crashing your app or giving you `NaN` (Not a Number) results.
...
xref:02_image_classification.adoc[Previous: Image Classification Concepts] | xref:04_ci_render_validation.adoc[Next: CI/CD Render Validation]
