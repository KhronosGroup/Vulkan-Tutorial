...
bool Renderer::buildAccelerationStructures(const std::vector<Entity *>& entities) {
  auto asStartTime = std::chrono::steady_clock::now();
  if (!accelerationStructureEnabled || !rayQueryEnabled) {
    std::cout << "Acceleration structures not supported on this device\n";
    return false;
  }

  // Large scenes (Bistro) take seconds/minutes to build in Debug. Suppress watchdog
  // so we don't abort, but we'll still update progress markers for user feedback.
  ScopedWatchdogSuppression watchdogGuard(this);
  watchdogProgressLabel.store("AS: buildAccelerationStructures start", std::memory_order_relaxed);
  ...
  try {
    ...
    auto kickWatchdog = [&](bool force = false) {
      auto now = std::chrono::steady_clock::now();
      if (force || now - lastKick > std::chrono::milliseconds(200)) {
    ...
    size_t readyUniqueMeshCount = 0; {
      ...
      for (Entity* entity : entities) {
        if (++processed % 100 == 0) kickWatchdog();
    ...
    // They will be deleted once the GPU has finished using them according to the timeline.
    if (!blasStructures.empty() || *tlasStructure.handle) {
      ...
      pendingDelete.timelineValue = currentTimelineValue;
    ...
    for (Entity* entity : entities) {
      if (++processedCount % 100 == 0) kickWatchdog();
      if (!entity || !entity->IsActive()) {
        skippedInactive++;
        continue;
      }
    ...
    // Batch build BLAS structures in smaller chunks to avoid TDR and excessive scratch allocation
    const size_t blasBatchSize = 128;
    for (size_t batchStart = 0; batchStart < uniqueMeshes.size(); batchStart += blasBatchSize) {
      size_t batchEnd = std::min(batchStart + blasBatchSize, uniqueMeshes.size());
      size_t currentBatchCount = batchEnd - batchStart;

      std::vector<vk::AccelerationStructureBuildGeometryInfoKHR> blasBuildInfos;
      std::vector<vk::AccelerationStructureGeometryKHR> blasGeometries;
      std::vector<vk::AccelerationStructureBuildRangeInfoKHR> blasRangeInfos;
      blasBuildInfos.reserve(currentBatchCount);
      blasGeometries.reserve(currentBatchCount);
      blasRangeInfos.reserve(currentBatchCount);

      vk::DeviceSize totalBatchScratchSize = 0;
      std::vector<vk::DeviceSize> batchScratchOffsets;
      batchScratchOffsets.reserve(currentBatchCount);

      // Alignment requirement for scratch data
      const vk::DeviceSize scratchAlignment = std::max<vk::DeviceSize>(
        accelStructProperties.minAccelerationStructureScratchOffsetAlignment, 128);

      for (size_t i = batchStart; i < batchEnd; ++i) {
        if (i % 100 == 0) kickWatchdog();
        ...
        buildInfo.pGeometries = &blasGeometries.back();
        ...
        vk::DeviceSize alignedScratchSize = (sizeInfo.buildScratchSize + scratchAlignment - 1) & ~(scratchAlignment - 1);
        batchScratchOffsets.push_back(totalBatchScratchSize);
        totalBatchScratchSize += alignedScratchSize;
        ...
        blasRangeInfos.push_back(rangeInfo);
      ...
      // Create a single shared scratch buffer for this batch
      if (totalBatchScratchSize > 0) {
        auto [scratchBuffer, scratchAlloc] = createBufferPooled(
          totalBatchScratchSize,
          vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress,
          vk::MemoryPropertyFlagBits::eDeviceLocal);
        vk::DeviceAddress scratchBaseAddress = getBufferDeviceAddress(device, *scratchBuffer);

        for (size_t j = 0; j < blasBuildInfos.size(); ++j) {
          blasBuildInfos[j].scratchData = scratchBaseAddress + batchScratchOffsets[j];
        }

        std::vector<const vk::AccelerationStructureBuildRangeInfoKHR*> pRangeInfos;
        pRangeInfos.reserve(blasRangeInfos.size());
        for (const auto& r : blasRangeInfos) {
          pRangeInfos.push_back(&r);
        }
        cmdBuffer.buildAccelerationStructuresKHR(blasBuildInfos, pRangeInfos);

        // Scratch buffer must stay alive until GPU is done with this batch.
        // For simplicity in this one-time build, we'll keep all batch scratch buffers alive
        // until the final fence wait at the end of the function.
    ...
    // Per-entity pre-calculated properties to avoid redundant string parsing in hot loops
    struct EntityASProperties {
    ...
    std::unordered_map<Entity*, EntityASProperties> entityASProps;
    entityASProps.reserve(renderableEntities.size());

    if (modelLoader) {
      materialByIndex.reserve(renderableEntities.size());
      static constexpr uint32_t kMaxSupportedMaterialIndex = 100000u;

      watchdogProgressLabel.store("AS: pre-calculate entity properties", std::memory_order_relaxed);
      uint32_t processedCount = 0;
      ...
      for (Entity* e : renderableEntities) {
        ...
        EntityASProperties& props = entityASProps[e];
        const std::string& name = e->GetName();

        // 1. Determine material index and Material*
        if (name.find("Ball_") == 0) {
          props.materialIndex = 9999u;
          props.material = modelLoader->GetMaterial("BallMaterial");
        } else {
          ...
          if (matPos != std::string::npos) {
            ...
            if (numEnd != std::string::npos) {
              try {
                props.materialIndex = static_cast<uint32_t>(std::stoi(name.substr(numStart, numEnd - numStart)));
                if (numEnd + 1 < name.size()) {
                    props.material = modelLoader->GetMaterial(name.substr(numEnd + 1));
                }
              } catch (...) {}
            }
          }
        ...
        if (props.material) {
            materialByIndex[props.materialIndex] = props.material;
        }
        ...
        if (nameLower.find("sky") != std::string::npos ||
          ...
          props.isEnvironment = true;
        }

        // 3. Safety check scale
        if (!props.isEnvironment) {
        ...
        // 4. Determine opacity from cache/material
        bool forceNoOpaqueCache = false;
        bool forceOpaqueCache = false;
        const Material* cachedMat = nullptr;

        auto itRes = entityResources.find(e);
        if (itRes != entityResources.end()) {
          ensureEntityMaterialCache(e, itRes->second);
          const MaterialProperties& mp = itRes->second.cachedMaterialProps;
          forceNoOpaqueCache = (mp.alphaMask > 0.5f) || itRes->second.cachedIsBlended || (mp.transmissionFactor > 0.01f) || itRes->second.cachedIsGlass || itRes->second.cachedIsLiquid;
          forceOpaqueCache = !forceNoOpaqueCache;
          cachedMat = itRes->second.cachedMaterial;
        }

        const Material* mat = props.material ? props.material : cachedMat;
        ...
        props.forceNoOpaque = forceNoOpaqueCache || forceNoOpaqueMat;
        props.forceOpaque = (!props.forceNoOpaque) && (!props.isEnvironment) && (forceOpaqueCache || forceOpaqueMat);
      }
    }
    ...
    watchdogProgressLabel.store("AS: TLAS build (instances)", std::memory_order_relaxed);

    for (auto entity : renderableEntities) {
      if (!entity) continue;

      const EntityASProperties& props = entityASProps[entity];
      auto meshComp = entity->GetComponent<MeshComponent>();
      ...
      for (size_t iInst = 0; iInst < instCount; ++iInst) {
        // Kick watchdog less frequently during instance loop
        if (runningInstanceIndex % 1000 == 0) {
            lastFrameUpdateTime.store(std::chrono::steady_clock::now(), std::memory_order_relaxed);
            watchdogProgressIndex.store(runningInstanceIndex, std::memory_order_relaxed);
        }

        glm::mat4 finalModel = entityModel;
        uint32_t resolvedMaterialIndex = props.materialIndex;
        ...
        if (hasInstance && iInst < meshInstCount) {
          ...
          resolvedMaterialIndex = id.materialIndex;
        }
        ...
        AS_Instance.instanceCustomIndex = runningInstanceIndex;
        ...
        GeometryInfo gi{};
        gi.vertexBufferAddress = getBufferDeviceAddress(device, *meshRes.vertexBuffer);
        gi.indexBufferAddress = getBufferDeviceAddress(device, *meshRes.indexBuffer);
        // Cache vertexCount from meshComp outside this hot inner loop if possible,
        // but for now ensure it's at least not a redundant function call if the compiler doesn't optimize it.
        const uint32_t vCount = static_cast<uint32_t>(meshComp->GetVertices().size());
        gi.vertexCount = vCount;
    ...
    SubmitToQueue2(*graphicsQueue, *cmdBuffer, false, nullptr, *fence);
    ...
    lastBuiltUniqueMeshCount = uniqueMeshes.size();
    lastASBuiltTlasInstanceCount = instanceCount;
    lastASBuildTime = std::chrono::steady_clock::now();
    ...
    auto asEndTime = std::chrono::steady_clock::now();
    auto asDurationMs = std::chrono::duration_cast<std::chrono::milliseconds>(asEndTime - asStartTime).count();
    return true;
...
bool Renderer::refitTopLevelAS(const std::vector<Entity *>& entities, CameraComponent* camera) {
  try {
    ...
    vk::raii::Fence fence(device, vk::FenceCreateInfo{});
    SubmitToQueue2(*graphicsQueue, *cmd, false, nullptr, *fence);
