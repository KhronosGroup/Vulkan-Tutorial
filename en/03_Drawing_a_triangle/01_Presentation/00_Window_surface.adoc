...
// START c
uint32_t queueIndex = ~0;
for (uint32_t qfpIndex = 0; qfpIndex < queueFamilyCount; qfpIndex++)
{
...
[source,multilang,c++,c]
.The complete createLogicalDevice function
----
// START c++
...
// START c
void createLogicalDevice() {
    // find the index of the first queue family that supports graphics and present
    ...
    // query for Vulkan 1.3 features, chaining pNext by hand
