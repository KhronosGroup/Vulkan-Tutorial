...
bool LoadGLTFModel(Engine* engine,
  ...
  try {
    ...
    // Async texture uploads will continue in the background.
    // Removing the blocking wait allows the engine to start quickly.
    // renderer->WaitForAllTextureTasks();
    ...
    renderer->AddLoadingWorkItems(materialMeshes.size());
    ...
    for (size_t meshIdx = 0; meshIdx < materialMeshes.size(); ++meshIdx) {
      ...
      if ((meshIdx % 64u) == 0u) {
        ...
        // Yield to main thread and kick watchdog to ensure responsiveness
        renderer->KickWatchdog();
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
      }
      renderer->CompleteLoadingWorkItems(1);
    ...
    if (!geometryEntities.empty()) {
      ...
      renderer->AddLoadingWorkItems(geometryEntities.size());
    ...
    if (!animations.empty()) {
      ...
      if (animController) {
        ...
        renderer->AddLoadingWorkItems(animatedNodeMeshes.size());
        ...
        for (const auto& [nodeIndex, meshIndex] : animatedNodeMeshes) {
          ...
          if (isFirstUse && sourceMaterialMeshIdx < geometryEntities.size()) {
            ...
            if (nodeEntity) {
              ...
              renderer->AddLoadingWorkItems(1);
  ...
  // Scene parsing is complete. Transition to Preallocating state so the render thread
  // can process the entity geometry queue before moving to PhysicsInit and Play.
  std::cout << "[Loading] Scene parsing complete. Transitioning to Preallocating." << std::endl;
  renderer->SetInternalLoadingState(Renderer::InternalLoadingState::Preallocating);
  // renderer->SetLoadingPhase(Renderer::LoadingPhase::Scene);
  renderer->SetLoadingPhaseProgress(1.0f);

  // Request acceleration structure build for later (will be gated by state machine in Render)
  if (renderer->GetRayQueryEnabled() && renderer->GetAccelerationStructureEnabled()) {
    renderer->RequestAccelerationStructureBuild("Scene loading complete");
  }

  // Note: Entity preallocations are drained on the render thread after loading completes.
  // The renderer will finalize the loading state (MarkInitialLoadComplete) once
  // all geometry resources are created.
