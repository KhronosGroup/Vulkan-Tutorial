...
class PhysicsSystem {
  public:
	 ...
	 * @param timelineValue The current timeline value to signal upon completion.
	 * @param frameIndex Optional frame slot index for resource rotation.
	 */
    void Update(std::chrono::milliseconds deltaTime, uint64_t timelineValue = 0, uint32_t frameIndex = 0);
  ...
  private:
    ...
    // Track dynamic bodies for simulation optimization
    mutable uint32_t lastDynamicBodyCount = 0;
    mutable bool hasDynamicBodies = false;
    mutable bool needsBroadPhase = true;
    ...
    struct VulkanResources {
      ...
      // Command buffers for compute operations (one per frame in flight)
    ...
    void SimulatePhysicsOnGPU(std::chrono::milliseconds deltaTime, uint64_t timelineValue = 0, uint32_t frameIndex = 0) const;
};
