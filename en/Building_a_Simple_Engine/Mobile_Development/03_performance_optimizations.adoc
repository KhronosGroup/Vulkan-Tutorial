...
[NOTE]
====
This chapter covers general mobile performance. For practices that arise specifically because the GPU is tile-based (TBR), see xref:04_rendering_approaches.adoc[Rendering Approaches: Tile-Based Rendering].
====
...
[NOTE]
====
If you are targeting tile-based GPUs (TBR), bandwidth can be heavily impacted by attachment load/store behavior and tile flushes. See xref:04_rendering_approaches.adoc[Rendering Approaches] — sections “Attachment Load/Store Operations on Tilers” and “Pipelining on Tilers: Subpass Dependencies and BY_REGION” for concrete guidance.
====
...
xref:02_platform_considerations.adoc[Previous: Platform Considerations] | xref:04_rendering_approaches.adoc[Next: Rendering Approaches]
