...
bool AudioSystem::LoadAudio(const std::string& filename, const std::string& name) {
  std::string resolvedPath = renderer->ResolvePath(filename);
  // Open the WAV file
  std::ifstream file(resolvedPath, std::ios::binary);
  if (!file.is_open()) {
    std::cerr << "Failed to open audio file: " << resolvedPath << " (original: " << filename << ")" << std::endl;
    return false;
  }
  ...
  if (std::strncmp(header.riff, "RIFF", 4) != 0 ||
    ...
    std::cerr << "Invalid WAV file format: " << resolvedPath << " (original: " << filename << ")" << std::endl;
    file.close();
    return false;
  }
  ...
  if (header.audioFormat != 1) {
    std::cerr << "Unsupported audio format (only PCM supported): " << resolvedPath << " (original: " << filename << ")" << std::endl;
    file.close();
    return false;
  }
  ...
  if (file.gcount() != static_cast<std::streamsize>(header.dataSize)) {
    std::cerr << "Failed to read complete audio data from: " << resolvedPath << " (original: " << filename << ")" << std::endl;
    return false;
  }
...
bool AudioSystem::LoadHRTFData(const std::string& filename) {
  ...
  if (!filename.empty()) {
    std::string resolvedPath = renderer->ResolvePath(filename);
    if (std::ifstream file(resolvedPath, std::ios::binary); file.is_open()) {
