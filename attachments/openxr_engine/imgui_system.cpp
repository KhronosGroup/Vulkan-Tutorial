...
void ImGuiSystem::UpdateTexture(ImTextureData* tex) {
  if (tex->Status == ImTextureStatus_WantDestroy) {
    // Release GPU resources and acknowledge so ImGui removes this entry next frame.
    fontImage = vk::raii::Image(nullptr);
    fontMemory = vk::raii::DeviceMemory(nullptr);
    fontView = vk::raii::ImageView(nullptr);
    fontSampler = vk::raii::Sampler(nullptr);
  ...
  if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
    ...
    try {
      ...
      // WantCreate: image is brand-new (Undefined). WantUpdates: image is already in ShaderReadOnlyOptimal.
      vk::Format format = (tex->BytesPerPixel == 4) ? vk::Format::eR8G8B8A8Unorm : vk::Format::eR8Unorm;
      vk::ImageLayout srcLayout = (tex->Status == ImTextureStatus_WantCreate)
