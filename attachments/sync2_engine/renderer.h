...
class Renderer {
  public:
	 ...
	 */
    bool Initialize(const std::string& appName, bool enableValidationLayers = true, bool debugSync = false);
    ...
    /**
     * @brief RAII helper to suppress watchdog aborts during known long-running operations.
     */
    struct ScopedWatchdogSuppression {
      Renderer* r;
      explicit ScopedWatchdogSuppression(Renderer* rr) : r(rr) {
        if (r)
          r->watchdogSuppressed.store(true, std::memory_order_relaxed);
      }
      ~ScopedWatchdogSuppression() {
        if (r)
          r->watchdogSuppressed.store(false, std::memory_order_relaxed);
      }
    };
    ...
    // Expose frame timeline semaphore and last value
    vk::Semaphore GetFrameTimeline() const {
      return *frameTimeline;
    }
    uint64_t GetCurrentTimelineValue() const {
      return totalFrameCount.load(std::memory_order_relaxed) * 10;
    }
    uint64_t GetNextTimelineValue() const {
      return totalFrameCount.load() + 1;
    }
    ...
    uint64_t GetNextUploadTimelineValue() {
      return nextUploadTimelineValue.fetch_add(1, std::memory_order_relaxed) + 1;
    }

    // Signal the frame timeline semaphore without submitting any work (used to break deadlocks)
    void SignalFrameTimeline(uint64_t value) {
    ...
    /**
	 * @brief Unified entry point for all queue submissions using Synchronization 2.
	 * @param queue The queue to submit to.
	 * @param submitInfo The submission info.
	 * @param fence Optional fence to signal.
	 */
    void Submit2(vk::Queue queue, vk::SubmitInfo2 submitInfo, vk::Fence fence = nullptr) {
      if (submitInfo.commandBufferInfoCount == 0 && submitInfo.signalSemaphoreInfoCount == 0 && submitInfo.waitSemaphoreInfoCount == 0 && !fence) return;

      std::lock_guard<std::mutex> lock(queueMutex);

      // Create local mutable copies of semaphore infos to ensure changes are seen by the driver
      std::vector<vk::SemaphoreSubmitInfo> signalInfos;
      if (submitInfo.signalSemaphoreInfoCount > 0 && submitInfo.pSignalSemaphoreInfos) {
        signalInfos.assign(submitInfo.pSignalSemaphoreInfos, submitInfo.pSignalSemaphoreInfos + submitInfo.signalSemaphoreInfoCount);

        VkSemaphore hFrame = (!!*frameTimeline) ? (VkSemaphore)*frameTimeline : VK_NULL_HANDLE;
        VkSemaphore hUploads = (!!*uploadsTimeline) ? (VkSemaphore)*uploadsTimeline : VK_NULL_HANDLE;

        for (auto& si : signalInfos) {
          VkSemaphore handle = (VkSemaphore)si.semaphore;
          if (handle == VK_NULL_HANDLE) continue;

      if (hFrame != VK_NULL_HANDLE && handle == hFrame) {
        uint64_t currentGpu = frameTimeline.getCounterValue();
        uint64_t lastSub = nextFrameTimelineValue.load(std::memory_order_relaxed);
        uint64_t val = std::max(currentGpu, lastSub);
        if (si.value <= val) {
          si.value = val + 1;
        }
        nextFrameTimelineValue.store(si.value, std::memory_order_relaxed);
      } else if (hUploads != VK_NULL_HANDLE && handle == hUploads) {
        uint64_t currentGpu = uploadsTimeline.getCounterValue();
        uint64_t lastSub = nextUploadTimelineValue.load(std::memory_order_relaxed);
        uint64_t val = std::max(currentGpu, lastSub);
        if (si.value <= val) {
          si.value = val + 1;
        }
        nextUploadTimelineValue.store(si.value, std::memory_order_relaxed);
      }
        }

        // IMPORTANT: Update the CALLER's memory so they know the assigned signal value.
        for (uint32_t i = 0; i < submitInfo.signalSemaphoreInfoCount; ++i) {
           const_cast<vk::SemaphoreSubmitInfo*>(submitInfo.pSignalSemaphoreInfos)[i].value = signalInfos[i].value;
        }

        // Re-point the submitInfo to our local updated array
        submitInfo.pSignalSemaphoreInfos = signalInfos.data();
      }

      // Use the C-style API directly via Vulkan-HPP to ensure the driver sees our pointers
      VkSubmitInfo2 vsi = static_cast<VkSubmitInfo2>(submitInfo);
      VkResult res = VULKAN_HPP_DEFAULT_DISPATCHER.vkQueueSubmit2(static_cast<VkQueue>(queue), 1, &vsi, static_cast<VkFence>(fence));
      if (res != VK_SUCCESS) {
        std::cerr << "Error: vkQueueSubmit2 failed! Result: " << vk::to_string(static_cast<vk::Result>(res)) << std::endl;
      }
    }

    /**
	 * @brief Submit a command buffer to the compute queue using Synchronization 2 and a signal timeline semaphore.
	 * @param commandBuffer The command buffer to submit.
	 * @param signalSemaphore The timeline semaphore to signal.
	 * @param signalValue The value to signal.
	 */
    void SubmitToComputeQueue2(vk::CommandBuffer commandBuffer, vk::Semaphore signalSemaphore, uint64_t signalValue) {
      ...
      if (*computeQueue) {
        Submit2(*computeQueue, submit2, nullptr);
      } else {
        Submit2(*graphicsQueue, submit2, nullptr);
      }
    }
    ...
    /**
	 * @brief Submit a command buffer to a queue using Synchronization 2 and an optional signal timeline semaphore.
	 * @param queue The queue to submit to.
	 * @param commandBuffer The command buffer to submit.
	 * @param signalUploadsTimeline Whether to signal the uploads timeline.
	 * @param outSignalValue Pointer to receive the signaled value.
	 * @param fence Optional fence to signal.
	 */
    void SubmitToQueue2(vk::Queue queue, vk::CommandBuffer commandBuffer, bool signalUploadsTimeline = false, uint64_t* outSignalValue = nullptr, vk::Fence fence = nullptr) {
      if (!commandBuffer) return;
      ...
      if (signalUploadsTimeline && !!*uploadsTimeline) {
        // Enforce strictly monotonic signals (will be handled by Submit2)
      ...
      vk::SubmitInfo2 submit2{
        ...
        .pSignalSemaphoreInfos = (signalCount > 0) ? &signalInfo : nullptr
      };

      Submit2(queue, submit2, fence);
      if (outSignalValue && signalCount > 0) {
          *outSignalValue = signalInfo.value;
      }
    }
    ...
    std::atomic<uint32_t> totalLoadingItems{0};
    std::atomic<uint32_t> completedLoadingItems{0};
    ...
    enum class InternalLoadingState {
      Parsing,        // Background thread loading GLTF
      Preallocating,  // Render thread creating GPU resources
      PhysicsInit,    // Initializing physics/base textures
      Play            // Game ready
    };
    ...
    void SetLoadingPhase(LoadingPhase phase) {
      if (loadingPhase.load(std::memory_order_relaxed) != static_cast<uint32_t>(phase)) {
    ...
    // Smooth monotonic progress tracking across multiple loaders/phases
    void AddLoadingWorkItems(uint32_t count) {
      totalLoadingItems.fetch_add(count, std::memory_order_relaxed);
    }
    void CompleteLoadingWorkItems(uint32_t count) {
      completedLoadingItems.fetch_add(count, std::memory_order_relaxed);
    }
    float GetGlobalLoadingProgress() const {
      uint32_t total = totalLoadingItems.load(std::memory_order_relaxed);
      ...
      uint32_t finalTotal = total + texSched;
      uint32_t finalDone = done + texDone;

      if (finalTotal == 0) return 0.0f;
      return std::clamp(static_cast<float>(finalDone) / static_cast<float>(finalTotal), 0.0f, 1.0f);
    }

    void MarkInitialLoadComplete() {
      initialLoadComplete.store(true, std::memory_order_release);
      SetLoadingPhase(LoadingPhase::Finalizing);
      loadingPhaseProgress.store(1.0f, std::memory_order_release);
    }
    void SetInternalLoadingState(InternalLoadingState state) {
      currentInternalLoadingState.store(state, std::memory_order_release);
    }
    InternalLoadingState GetInternalLoadingState() const {
      return currentInternalLoadingState.load(std::memory_order_acquire);
    }
    bool IsLoading() const {
      // Stay in loading state until:
      // 1. The state machine hits Play
      // 2. The geometry preallocation queue is empty
      // 3. All pending mesh uploads (vertex/index data) are finished
      // This ensures we keep the high preallocation budget (100/frame) and blocking UI until everything is truly ready on the GPU.
      return currentInternalLoadingState.load(std::memory_order_relaxed) != InternalLoadingState::Play ||
    ...
    void SetLoading(bool v) {
      std::cout << "DEBUG: SetLoading(" << (v ? "true" : "false") << ")" << std::endl;
      loadingFlag.store(v, std::memory_order_release);
      if (v) {
        currentInternalLoadingState.store(InternalLoadingState::Parsing, std::memory_order_release);
        initialLoadStarted.store(true, std::memory_order_relaxed);
        initialLoadComplete.store(false, std::memory_order_release);
        SetLoadingPhase(LoadingPhase::Scene);
        loadingPhaseProgress.store(0.0f, std::memory_order_relaxed);
      } else {
        // Switch from Parsing to Preallocating
        if (currentInternalLoadingState.load() == InternalLoadingState::Parsing) {
          currentInternalLoadingState.store(InternalLoadingState::Preallocating, std::memory_order_release);
        }
      }
    }
    ...
    bool HasPendingPreallocations() const {
      return pendingEntityPreallocQueued.load(std::memory_order_relaxed);
    }
  ...
  public:
    // Milestone values for frameTimeline
    struct TimelineMilestones {
        static constexpr uint64_t eFrameStart = 0;
        static constexpr uint64_t ePhysicsFinished = 1;
        static constexpr uint64_t eCullingFinished = 2;
        static constexpr uint64_t eGpuWorkFinished = 3;
    };
  ...
  private:
    ...
    uint64_t currentTimelineValue{0}; // The value being signaled by the current frame's GPU work
    std::atomic<uint64_t> lastCompletedFrame{0};

    // Monotonic trackers for timeline signals
    std::atomic<uint64_t> nextFrameTimelineValue{0};
    std::atomic<uint64_t> nextUploadTimelineValue{0};
    ...
    vk::PipelineRenderingCreateInfo rayQueryPipelineRenderingCreateInfo;
    ...
    // Tracks last timeline value that has been submitted for signaling on uploadsTimeline (obsolete, using nextUploadTimelineValue)
    ...
    struct PendingASDelete {
      ...
      uint64_t timelineValue = 0; // The timeline value when this AS can be safely deleted
    };
    ...
    mutable std::mutex pendingMeshUploadsMutex;
    ...
    bool HasPendingMeshUploads() const {
      std::lock_guard<std::mutex> lk1(pendingMeshUploadsMutex);
      if (!pendingMeshUploads.empty()) return true;
      std::lock_guard<std::mutex> lk2(inFlightMeshUploadsMutex);
      return !inFlightMeshUploads.empty();
    }
    ...
    // When true, initial scene load has started
    std::atomic<bool> initialLoadStarted{false};
    ...
    std::atomic<InternalLoadingState> currentInternalLoadingState{InternalLoadingState::Parsing};
    ...
    // Protect concurrent access to entityResources
    mutable std::shared_mutex entityResourcesMutex;

    // Protect concurrent access to meshResources
    mutable std::shared_mutex meshResourcesMutex;
    ...
    struct EntityResources {
      ...
      // Tracks last updated frame for descriptor sets to avoid redundant per-frame updates.
      // size = MAX_FRAMES_IN_FLIGHT. Initialized to 0xFFFFFFFFFFFFFFFF.
      std::vector<uint64_t> lastUpdatedFrameBasic;
      std::vector<uint64_t> lastUpdatedFramePBR;
    ...
    size_t lastBuiltUniqueMeshCount = 0;
    std::chrono::steady_clock::time_point lastASBuildTime = std::chrono::steady_clock::now() - std::chrono::hours(1);
    ...
    // Private methods
    bool createInstance(const std::string& appName, bool enableValidationLayers, bool debugSync);
