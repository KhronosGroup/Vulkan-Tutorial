...
bool Engine::Initialize(const std::string& appName, int width, int height, bool enableValidationLayers, bool debugSync) {
  // Create platform
  ...
  if (!renderer->Initialize(appName, enableValidationLayers, debugSync)) {
    return false;
  }
...
void Engine::Update(TimeDelta deltaTime) {
  ...
  const bool loading = renderer && renderer->IsLoading();
  ...
  // Derive the next frame's base timeline value from the renderer's CURRENT timeline
  // to avoid races where Render advanced farther than our "next" snapshot.
  uint64_t currentTimeline = renderer ? renderer->GetCurrentTimelineValue() : 0;
  // Ensure we round to the next available 10-unit base.
  uint64_t nextFrameBaseValue = ((currentTimeline / 10ULL) + 1ULL) * 10ULL;
  uint64_t physicsSignalValue = nextFrameBaseValue + Renderer::TimelineMilestones::ePhysicsFinished;

  physicsSystem->Update(deltaTime, physicsSignalValue, static_cast<uint32_t>(nextFrameBaseValue / 10ULL));
...
bool Engine::InitializeAndroid(android_app* app, const std::string& appName, bool enableValidationLayers, bool debugSync) {
  // Create platform
