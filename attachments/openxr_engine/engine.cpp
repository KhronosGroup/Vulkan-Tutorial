...
bool Engine::Initialize(const std::string& appName, int width, int height, bool enableValidationLayers, bool useXR) {
  // Create platform
  ...
  if (!renderer->Initialize(appName, enableValidationLayers, useXR)) {
    return false;
  }
...
void Engine::Update(XrTime predictedTime) {
  if (renderer && renderer->IsXrMode()) {
    auto& xrContext = renderer->GetXrContext();

    // 1. Poll OpenXR action states (Grab, Teleport, etc.) (Chapter 7)
    xrContext.pollActions();

    // 2. Retrieve the predicted views (poses and FOVs) from OpenXR (Chapter 5)
...
void Engine::Render(XrTime predictedTime) {
  ...
  // Render the scene for XR (per-eye)
  renderer->Render(snapshot, activeCamera, imguiSystem.get(), predictedTime);
}
...
void Engine::UpdateFPSWindowTitle() {
  ...
  std::string title = (renderer && renderer->IsXrMode() ? "Simple Engine [XR] - Frame: " : "Simple Engine - Frame: ") +
...
bool Engine::InitializeAndroid(android_app* app, const std::string& appName, bool enableValidationLayers, bool useXR) {
  // Create platform
